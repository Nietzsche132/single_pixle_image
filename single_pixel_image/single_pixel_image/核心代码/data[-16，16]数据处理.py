...
data_number = 2
# 数据目录
data_dir = r"D:\桌面\毕设\data[-16，16]\data4"
# 输入文件路径
...
# 定义fx和fy的组合（-16到16）
fx_fy_pairs = []
# 生成fx从-16到16，fy从0到16的所有组合
for fx in range(-16, 17):  # fx=-16,-15,...,16
...
# 创建33x33的傅里叶系数矩阵（fx和fy从-16到16）
fourier_matrix_33x33 = np.zeros((33, 33), dtype=complex)
...
for i, row in result_df.iterrows():
    ...
    # 将fx和fy从[-16,16]映射到[0,32]
    fx_idx = fx + 16
    fy_idx = fy + 16
    
    # 确保fx和fy在有效范围内
    if 0 <= fx_idx < 33 and 0 <= fy_idx < 33:
        # 获取傅里叶系数
        ...
        if pd.notna(fourier_str) and fourier_str != '':
            ...
            if 'j' in fourier_str:
                ...
                if '+' in fourier_str:
                    ...
                    imag_part = -float(parts[1].replace('j', ''))
                # 交换fx和fy的顺序，解决图像翻转问题
                fourier_matrix_33x33[fy_idx, fx_idx] = complex(real_part, imag_part)
            else:
                # 只有实部的情况
                # 交换fx和fy的顺序，解决图像翻转问题
                real_part = float(fourier_str)
                fourier_matrix_33x33[fy_idx, fx_idx] = complex(real_part, 0)

# 处理(0,0)位置的特殊情况（直流分量）已经在前面的循环中处理，这里不再重复处理

# 应用傅里叶变换的共轭对称性 F(u,v) = F*(-u,-v)
print("应用傅里叶变换的共轭对称性...")
for fx in range(-16, 17):
    for fy in range(-16, 17):
        # 计算当前频率分量的索引
        fx_idx = fx + 16
        fy_idx = fy + 16
        ...
        # 确保当前位置有值
        if 0 <= fx_idx < 33 and 0 <= fy_idx < 33:
            current_value = fourier_matrix_33x33[fy_idx, fx_idx]
            ...
            if (fx, fy) != (0, 0) and current_value != 0:
                ...
                # 计算共轭分量的索引
                conjugate_fx_idx = conjugate_fx + 16
                conjugate_fy_idx = conjugate_fy + 16
                
                # 确保共轭分量位置在有效范围内
                if 0 <= conjugate_fx_idx < 33 and 0 <= conjugate_fy_idx < 33:
                    # 设置共轭分量为当前分量的共轭复数
                    fourier_matrix_33x33[conjugate_fy_idx, conjugate_fx_idx] = np.conj(current_value)
...
# 填充128x128矩阵，只在fx和fy为-16到16的位置设置值
for fx in range(-16, 17):
    for fy in range(-16, 17):
        # 将fx和fy从[-16,16]映射到[0,32]
        fx_idx = fx + 16
        ...
        if 0 <= fx_128 < image_size and 0 <= fy_128 < image_size:
            # 直接使用fx和fy作为索引，不交换顺序
            fourier_matrix_128x128[fy_128, fx_128] = fourier_matrix_33x33[fy_idx, fx_idx]
...
# 创建放大的傅里叶系数图像（每个像素放大6倍，以保持合理的图像大小）
amplified_size = 33 * 6
amplified_fft = np.zeros((amplified_size, amplified_size), dtype=np.uint8)
for i in range(33):
    for j in range(33):
        # 反转行索引，使 fy=0 对应图像顶部，fy=16 对应图像中心
        # 这样直流分量（fx=0, fy=0）就会显示在图像中心
        row = 32 - i
        amplified_fft[row*6:(row+1)*6, j*6:(j+1)*6] = fourier_amplitude_normalized[i, j]
