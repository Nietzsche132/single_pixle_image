...
# 定义fx和fy的组合（-32到32）
fx_fy_pairs = []
# 生成fx从-32到32，fy从0到32的所有组合
for fx in range(-32, 33):  # fx=-32,-31,...,32
    for fy in range(33):  # fy=0,1,2,...,32
...
# 计算归一化因子（fx=fy=0时的前两个值平均值减去后两个值平均值）
...
for i in range(len(result_df)):
    ...
    if fx == 0 and fy == 0:
        ...
        d3 = row['D3 (180°)']
        d4 = row['D4 (270°)']
        if pd.notna(d1) and pd.notna(d2) and pd.notna(d3) and pd.notna(d4) and not isinstance(d1, str) and not isinstance(d2, str) and not isinstance(d3, str) and not isinstance(d4, str):
            # 计算前两个值的平均值
            avg_first_two = (float(d1) + float(d2)) / 2
            # 计算后两个值的平均值
            avg_last_two = (float(d3) + float(d4)) / 2
            # 计算归一化因子
            normalization_factor = avg_first_two - avg_last_two
...
for i in range(len(result_df)):
    ...
    if pd.notna(d1_d3_val) and pd.notna(d2_d4_val):
        ...
        if normalization_factor != 0:
            d1_d3_val /= normalization_factor
            d2_d4_val /= normalization_factor
        
        # 处理fx=fy=0的情况，只保留实部
        if fx == 0 and fy == 0:
            fourier = f"{d1_d3_val:.5f}"
        else:
            # 傅里叶系数 = D1-D3 - j*(D2-D4)
            # 处理符号，将"- -"替换为"+"
            if d2_d4_val < 0:
...
# 创建65x65的傅里叶系数矩阵（fx和fy从-32到32）
fourier_matrix_33x33 = np.zeros((65, 65), dtype=complex)
...
for i, row in result_df.iterrows():
    ...
    # 将fx和fy从[-32,32]映射到[0,64]
    fx_idx = fx + 32
    fy_idx = fy + 32
    
    # 确保fx和fy在有效范围内
    if 0 <= fx_idx < 65 and 0 <= fy_idx < 65:
        # 获取傅里叶系数
        ...
        if pd.notna(fourier_str) and fourier_str != '':
            ...
            if 'j' in fourier_str:
                ...
                if '+' in fourier_str:
                    ...
                    real_part = float(parts[0])
                    imag_part = -float(parts[1].replace('j', ''))
                # 不交换fx和fy的顺序，直接使用原始顺序
                fourier_matrix_33x33[fx_idx, fy_idx] = complex(real_part, imag_part)
            else:
                # 只有实部的情况
                # 不交换fx和fy的顺序，直接使用原始顺序
                real_part = float(fourier_str)
                fourier_matrix_33x33[fx_idx, fy_idx] = complex(real_part, 0)

# 处理(0,0)位置的特殊情况（直流分量）已经在前面的循环中处理，这里不再重复处理

# 应用傅里叶变换的共轭对称性 F(u,v) = F*(-u,-v)
print("应用傅里叶变换的共轭对称性...")
for fx in range(-32, 33):
    for fy in range(-32, 33):
        # 计算当前频率分量的索引
        fx_idx = fx + 32
        fy_idx = fy + 32
        ...
        # 确保当前位置有值
        if 0 <= fx_idx < 65 and 0 <= fy_idx < 65:
            current_value = fourier_matrix_33x33[fx_idx, fy_idx]
            
            # 只处理非零值且不是直流分量的情况
            if (fx, fy) != (0, 0) and current_value != 0:
                # 计算共轭频率分量
                conjugate_fx = -fx
                conjugate_fy = -fy
                
                # 计算共轭分量的索引
                conjugate_fx_idx = conjugate_fx + 32
                conjugate_fy_idx = conjugate_fy + 32
                
                # 确保共轭分量位置在有效范围内
                if 0 <= conjugate_fx_idx < 65 and 0 <= conjugate_fy_idx < 65:
                    # 检查共轭分量位置是否已有值
                    if fourier_matrix_33x33[conjugate_fx_idx, conjugate_fy_idx] == 0:
                        # 只有当共轭分量位置为空时才设置
                        fourier_matrix_33x33[conjugate_fx_idx, conjugate_fy_idx] = np.conj(current_value)

print("33x33傅里叶系数矩阵：")
print(fourier_matrix_33x33)

# 创建128x128的傅里叶系数矩阵
image_size = 128
fourier_matrix_128x128 = np.zeros((image_size, image_size), dtype=complex)

# 填充128x128矩阵，只在fx和fy为-32到32的位置设置值
for fx in range(-32, 33):
    for fy in range(-32, 33):
        # 将fx和fy从[-32,32]映射到[0,64]
        fx_idx = fx + 32
        fy_idx = fy + 32
        # 计算在128x128矩阵中的位置
        # 标准FFT格式：零频在左上角，正频率向右，负频率在右侧
        if fx >= 0:
            # 正频率：从左侧开始
            fx_128 = fx
        else:
            # 负频率：从右侧开始
            fx_128 = image_size + fx
        ...
        if fy >= 0:
            ...
            # 负频率：从底部开始
            fy_128 = image_size + fy
        
        # 确保位置在有效范围内
        if 0 <= fx_128 < image_size and 0 <= fy_128 < image_size:
            # 直接使用fx和fy作为索引，不交换顺序
            fourier_matrix_128x128[fy_128, fx_128] = fourier_matrix_33x33[fx_idx, fy_idx]

# 进行逆傅里叶变换
reconstructed_image = np.fft.ifft2(fourier_matrix_128x128)

# 翻转图像以纠正左右反转问题
# 注意：这里可能不需要翻转，或者需要不同的翻转方式
...
# 左右翻转图像
reconstructed_image_real = np.fliplr(reconstructed_image_real)
...
# 保存傅里叶系数幅度图（使用PIL）
fourier_amplitude = np.abs(fourier_matrix_33x33)
# 归一化到0-255
...
# 创建放大的傅里叶系数图像（每个像素放大3倍，以保持合理的图像大小）
amplified_size = 65 * 3
amplified_fft = np.zeros((amplified_size, amplified_size), dtype=np.uint8)
for i in range(65):
    for j in range(65):
        # 反转行索引，使 fy=0 对应图像顶部，fy=32 对应图像中心
        # 这样直流分量（fx=0, fy=0）就会显示在图像中心
        row = 64 - j
        amplified_fft[row*3:(row+1)*3, i*3:(i+1)*3] = fourier_amplitude_normalized[i, j]
...
try:
    ...
    # 计算SSIM
    ssim_value = ssim(original_array, reconstructed_image_normalized, data_range=255)
    ...
    # 添加MSE、PSNR和SSIM（在右下角，带背景框）
    mse_psnr_text = f"MSE: {mse:.2f}, PSNR: {psnr_value:.2f} dB, SSIM: {ssim_value:.4f}"
    ...
    box_padding = 10
    # 减小右边距，使文本框更靠右
    right_margin = -3  # 使用负的右边距，使文本框更靠右
    box_x = comparison_width - text_width - box_padding * 2 - right_margin
